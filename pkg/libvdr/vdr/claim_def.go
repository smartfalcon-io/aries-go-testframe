...
func NewClaimDef(from string, ref uint32,tag string, primary, revocation map[string]interface{}) *Request {
	// primary is publickey of issuer did
	return &Request{
		Operation: ClaimDef{
...
func (r *Client) CreateClaimDef(from string, ref uint32,tag string, pubKey, revocation map[string]interface{}, signer Signer) (string, error) {
	// from is issuerdid 
	claimDef := NewClaimDef(from, ref,tag, pubKey, revocation)
